...
export default function QuotesPage() {
  ...
  const handleSaveQuotation = async (savedQuotation: Quotation | null) => {
    const wasEditing = !!currentQuotation;

    setIsEditing(false);
    setCurrentQuotation(null);
    fetchQuotations(filters);

    if (wasEditing && savedQuotation) {
      setTimeout(() => {
        setCurrentQuotation(savedQuotation);
        setIsEditing(true);
        addNotification(
          "info",
          `Se creó la cotización #${savedQuotation.id}. Puedes continuar editándola.`
        );
      }, 300);
    }
  };
